...
class Log():
    def __init__(self):
        self._log = uuid.uuid4()

    def create_log_table(self, cur: psycopg.Cursor) -> None:
        cur.execute("create table if not exists public.log (id bigint not null generated by default as identity primary key, run uuid, name text, start timestamptz not null default clock_timestamp(), stop timestamptz)")

    def start(self, conn: psycopg.Connection, name: str) -> int:
        with conn.cursor() as cur:
            cur.execute("insert into public.log (name) values (%s) returning id", (name, ))
            return int(cur.fetchone()[0])
    
    def stop(self, conn: psycopg.Connection, id: int) -> None:
        with conn.cursor() as cur:
...
class TSVector(BaseANN):
    ...
    def load_table(self, X: numpy.array) -> None:
        id = 0
        with self._pool.connection() as conn:
            id = self._log.start(conn, "loading table")
    ...
    def index_chunks(self, chunks: list[str]) -> None:
        id = 0
        with self._pool.connection() as conn:
            id = self._log.start(conn, "indexing")
        if len(chunks) == 0:
            return
    ...
    def fit(self, X: numpy.array) -> None:
        ...
        with psycopg.connect(self._connection_str) as conn:
            with conn.cursor() as cur:
                ...
                self._log.create_log_table(conn)
        self.start_pool()
