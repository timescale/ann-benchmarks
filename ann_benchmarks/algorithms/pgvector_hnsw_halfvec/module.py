...
class PGVectorHNSWHalfvec(BaseANN):
    ...
    def shared_buffers(self, conn: psycopg.Connection) -> bool:
        ...
        with conn.cursor() as cur:
            ...
            sql_query = sql_query.replace("%s", "$1", 1)
            sql_query = sql_query.replace("%s", "$2", 1)
            cur.execute(f"""
                        select 
    ...
    def index_table(self) -> None:
        with self._pool.connection() as conn:
            with conn.cursor() as cur:
                cur.execute(f"""create index on only public.items using hnsw ((embedding::halfvec({self._dimensions})) halfvec_cosine_ops)
    ...
    def index_chunk(self, chunk: str) -> Optional[Exception]:
        try:
            with self._pool.connection() as conn:
                with conn.cursor() as cur:
                    cur.execute(f"""create index on only {chunk} using hnsw ((embedding::halfvec({self._dimensions})) halfvec_cosine_ops) 
                        with (m = {self._m}, ef_construction = {self._ef_construction})"""
                    )
                    conn.commit()
    ...
    def fit(self, X: numpy.array) -> None:
        ...
        self._query = f"""SELECT id FROM public.items ORDER BY embedding::halfvec({self._dimensions}) <=> %s::halfvec({self._dimensions}) LIMIT %s"""
        print(self._query)
    ...
    def __str__(self):
        return f"PGVectorHNSWHalfvec(m={self._m}, ef_construction={self._ef_construction}, ef_search={self._ef_search})"
