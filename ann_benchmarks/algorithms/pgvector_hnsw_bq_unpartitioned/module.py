...
class PGVectorHNSWBQUnpartitioned(BaseANN):
    ...
    def does_index_exist(self, conn: psycopg.Connection) -> bool:
        ...
        with conn.cursor() as cur:
            cur.execute("""
                ...
                and tablename = 'items'
                and indexname like '%_binary_quantize_%'
                """)
    ...
    def fit(self, X: numpy.array) -> None:
        ...
        with self._pool.connection() as conn:
            index_exists = self.does_index_exist(conn)
        if not index_exists:
            self.index_table()
    ...
    def __str__(self):
        return f"PGVectorHNSQBQUnpartitioned(m={self._m}, ef_construction={self._ef_construction}, ef_search={self._ef_search})"
